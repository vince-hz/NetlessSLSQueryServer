...
ssh -f root@$IP "cd $DOWNLOAD_PATH && mv $NAME .. && mv env.json .. && netstat -tunlp|grep $PORT | awk '{print \$7}' | grep -o '[0-9]*' | xargs -r kill && nohup $SERVER_DIR_PATH/$NAME release > $SERVER_DIR_PATH/sls_server.log 2>&1 &"
rm $NAME